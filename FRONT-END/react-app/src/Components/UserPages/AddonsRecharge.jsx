...
const AddonsRecharge = () => {
  ...
  const handleRecharge = async() => {
    ...
    console.log(cno);
    try {
      await axios.post(`http://localhost:8080/api/recharges/addOn/${cno}/${cid}`, selectedPlan, {
        headers: {
      ...
      <br/>
        <div className="container">
          ...
          {selectedPlan && (
            ...
            <br/>
                <p style={{ color: "#333" }}>Id: {selectedPlan.addonId}</p>
                <p style={{ color: "#333" }}>Plan: {selectedPlan.addonName}</p>
                <p style={{ color: "#333" }}>Validity: {selectedPlan.addonValidity}</p>
                <p style={{ color: "#333" }}>Price: Rs. {selectedPlan.addonPrice}</p>
                <p style={{ color: "#333" }}>Additional benefits: {selectedPlan.addonDetails}</p>
                </>
          )}
          </div>
